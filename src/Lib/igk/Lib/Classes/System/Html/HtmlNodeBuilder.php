...
class HtmlNodeBuilder
{
    ...
    public function __invoke($data, ?HtmlItemBase $target = null)
    {
    ...
    /**
     * parse item to node builder
     * @param HtmlItemBase $n 
     * @return void 
     */
    public static function Generate(HtmlItemBase $n){
        $g = new HtmlVisitor($n);
        $sb = new StringBuilder;
        $tab = [];
        $ctab = null;
        $skip_space = false;
        $g->startVisitorListener = function(HtmlItemBase $t, $first_childs, $has_child, $last_child)use($sb, & $tab, & $ctab, & $skip_space){
        ...
        $g->endVisitorListener = function(HtmlItemBase $t)use($sb, & $tab, & $ctab){
            if ($ctab){
                $ctab = $ctab->parent;
            }
        };
        ...
        return sprintf('$builder(%s);', igk_array_dump_short($tab, true));
    ...
    static function _Loop($visitor, HtmlItemBase & $n, $q, & $keys, & $next, & $list, &$v_chain_info, & $_last, $_is_php8){
        ...
        while (!$next && (count($keys) > 0)) {
            ...
            if (!$v_from_key) {
                ...
                if ((strpos($k, self::KEY_CUSTOM_TARGET_PREFIX) === 0)) {
                    ...
                    // :: invoke the function last create item.
                    // :: @invoke the function name to the parent target item 
                    $k = trim(substr($k, 2));
                    $target_fc = $_last;
                    if ($k[0] == '@') {
                        $k = ltrim($k, '@');
