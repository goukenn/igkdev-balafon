...
class GenSelfSignedCertCommand extends AppExecCommand{
    ...
    */
    public function exec($command, string $public='public.crt', string $private='server_key.crt'){ 
		...
		$o = shell_exec("openssl genrsa -aes256 -out keyfile.crt 2048 2>&2 1>&2");
		Logger::print($o);
		Logger::info('generate public key');
		$o = shell_exec("openssl rsa -in keyfile.crt -out {$public} 2>&2 1>&2");
		Logger::print($o);
		Logger::info('generate server key');
		$o = shell_exec("openssl req -new -x509 -nodes -sha256 -key {$public}  -out {$private} 2>&2 1>&2");
		Logger::print($o);
		Logger::info('done');
	}
}