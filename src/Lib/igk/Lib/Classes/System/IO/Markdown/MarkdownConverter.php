    ...
    */
    ...
    protected function _treat_image($v, $captures = null)
    {
        ...
        if ($captures) {
            $c = $captures->beginCaptures['text'][0];
            $u = $captures->beginCaptures['uri'][0];
            if ((0 === strpos($u, './')) && $this->baseURL) {
                $u = igk_uri(Path::Combine($this->baseURL, substr($u, 2)));
            }
            $a["src"] = $u;
            $a['alt'] = $c;
        }
    ...
    protected function _treat_fence_code($v, $captures = null)
    {
        ...
        $v_service_name = IGKServices::CORE_CODE_HIGHLIGHT;
        if (!empty($v)) {
            $this->_updatePreviousOutput();
            $this->m_ltrim = true;
            $this->setIsSingleDefinition(false);
            if ($this->formatCodeBlock) {
                ...
                if (is_null($engine)) {
                    ...
                    $heighlight = igk_app()->getService($v_service_name) ?? igk_die(sprintf('[%s]- %s', $v_service_name, 'core code highlight is missing'));
