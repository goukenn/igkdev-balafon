            ...
            foreach($tab as $k=>$v){
                if(is_object($v)){
                    ...
                    switch($v_type){
                        ...
                        break;default: $li->addInput($k, "text", $defaultv);
                        break;
                    }
                }
                else{
...
EOF;
    ...
    private function _viewCtrlEditResult($v_dv){
        if(!($c=$this->SelectedController))
            return;
        $txb=$v_dv->addCol("igk-col-3-3")->addColViewBox()->addComponent($this, 
        HtmlComponents::AJXTabControl, "view_result", 1);
...
EOF;
    ...
    public function ca_edit_ctrl_ajx($oldcontent=null){
        ...
        if(file_exists($f)){
            ...
            $ul=$frm->add("ul");
            $v_txtarea=$ul->addLi()->addDiv()->setClass("fitw overflow-x-a")->addTextArea("clContent", ($oldcontent == null) ? $str: $oldcontent)->setClass("igk-php-code")->setAttribute("spellcheck", "false")->setAttribute("cols", 40)->setAttribute("rows", 30);
            $frm->addInput("clfile", "hidden", base64_encode(urlencode($f)));
            $frm->addInput("clframe", "hidden", $frame["id"]);
            $frm->addHSep();
            $frm->addBtn("btn_update", __("Update"));
    ...
    public function ca_tabv_ajx(){
        ...
        switch($g){
            ...
            if($c){
                ...
                $n->addDiv()->setClass("igk-danger")->Content=__("/!\\ No template found");
            }
            break;default: 
            $frm=$n->form();
            $frm->panel()->div()->Content=__("/!\\ Not Implement");
    ...
    public function ca_update_ctrl_properties(){
        ...
        foreach($c as $k=>$v){
            ...
            switch($k){
                ...
                break;default: $ctrl->Configs->$k=$s;
                break;
            }
        }
