 ...
 * @param string $file
 * @param ICssResourceResolver $doc
 * @param mixed $host
 ...
 * @return mixed
 */
function igk_cache_array_content(string $m, $fc = null)
{
 ...
 * @param mixed $mergescript
 * @param mixed $resregex
 * @return mixed
 */
function igk_cache_gen_cache(string $sourcedir, string $cachedir, $mergescript = null, $resregex = "/\.(js(on)?)$/")
{
    ...
    if ($tf = igk_io_getfiles($sourcedir, $rgx)) {
        foreach ($tf as $file) {
            ob_start();
...
function igk_cache_handle($uri = null)
{
    ...
    if (igk_io_file_exists($file = $fs->getCacheFilePath($uri))) {
        ob_start();
        readfile($file);
 ...
 * @param mixed $d
 * @param mixed $k
 * @param mixed $v node where to add object
 * @return mixed
 */
 ...
 * @return void
 */
function igk_conf_bind(closure $bindv, array $tab, string $name, $node)
{
    foreach ($tab as $v) {
        $n = $node->add($name);
        $bindv($n, $v);
...
function igk_config_php_index_content()
{
    ...
    $date = date("Ymd m:i:s");
    $conf_path = igk_server()->getConfigurationPath();
    $author = IGK_AUTHOR;
    $v = <<<ETF
<?php
// @file: index.php
...
\$_SERVER["PHP_SELF"] = \$_SERVER["REDIRECT_URL"] = "{$conf_path}";
 ...
 * @param array $index_or_args index of arguments
...
function igk_css_balafon_index(string $dir, $debug = null, ?bool $minfile = null)
{
    ...
    if (!defined("IGK_INIT") && empty($sess_id)) {
        $app = IGKApplication::Boot(IGKApplication::CSS_TYPE);
 ...
 * @return mixed
 */
function igk_css_bind_file(HtmlDocTheme $theme, ?BaseController $ctrl, ?string $file = null)
{
    $autoload = false;
    $layout = null;
    if (is_null($file) && $ctrl) {
        $layout = $ctrl->getViewLayout();
    ...
    if (defined("IGK_FORCSS")) {
        ...
        if ($layout->isStyleLoaded($f)) {
            return;
        }
    }
    \IGK\System\Html\Css\CssUtils::Include($f, $ctrl, $theme);
    $layout && $layout->styleLoaded($f);
}
/**
 ...
 * @param ?string $files
 * @example `default.pcss[|ctrl];main.pcss`
 * @var HtmlDocTheme $them
 * @var ?string $files ; separated theme files
 * @return mixed
 */
        ...
        if (is_string($v)) {
            ...
            if (is_array($tab)) {
                foreach ($tab as $s => $t) {
                    ...
                    $ok = 0;
                    $fdd = "";
                    if (igk_io_file_exists($t->File)) {
                        $ok = 1;
                        $fdd = igk_io_basepath($t->File);
                    } else {
                        $fdd = ($ok = igk_io_file_exists(igk_io_syspath($t->File))) ? $t->File : null;
                    }
                    if ($ok) {
...
function igk_css_ob_get_tempfile($f, &$from = null)
{
    ...
    IGKOb::Start();
    igk_css_bind_file($vtemp, null, $f);
    $m = IGKOb::Content();
    $out = "";
    IGKOb::Clear();
    if (!empty($m)) {
 ...
 * @param IGK\Css\ICssStyleContainer $theme the theme
...
function igk_css_reg_global_style_file(
    ...
    if (igk_io_file_exists($s) && preg_match("/(pgcss|css|pcss)/", igk_io_path_ext($s))) {
...
/**
 * helper: register color to current theme
 * @param mixed $clname
 * @param mixed $value
 * @param mixed $global
 * @param mixed $override
 ...
 * @param ?\IGK\Css\ICssStyleContainer $systheme current theme
 * @return mixed
 */
...
function igk_css_treat_bracket($v, $theme, $systheme = null, $gtheme = null, $doc = null)
{
    ...
    while (($c = preg_match_all($reg2, $v, $match))) {
        for ($i = 0; $i < $c; $i++) {
            ...
            //igk_wln_e(__FILE__.":".__LINE__ , 'subtreat bracked');
 ...
 * @param bool $cssRendering mode for direct rendering
 * @param bool $temp temp
 * @return mixed
 */
...
/**
 * get view args current document
 * @return mixed
 */
...
function igk_ctrl_include_content($ctrl, $file, $t, $params, $source = null, $dir = null)
{
    ...
    if ($c) {
        if ($dir == null)
            $dir = dirname($file);
        $f = $dir . "/{$c}.phtml";
        if (($f != $file) && igk_io_file_exists($f)) {
...
{
    if (!igk_io_file_exists($filepath, true))
        return null;
...
function igk_data_addcron($ctrl)
{
    ...
    igk_io_save_file_as_utf8_wbom($f, json_encode($t), true);
}
/**
 * auto generate doc.
 * @return mixed
 ...
 * @return mixed
 */
function igk_date_display_date($date, $format = null)
{
    $format = $format ?? igk_configs()->date_format ?? 'Y/m/d';
    return date_format(date_create($date), $format);
}
/**
 * helper:: backup controller data as xml object
 * @param mixed $ctrl
 * @param mixed $defentries
 * @return mixed
 */
 ...
 * @return mixed
 */
function igk_db_create_datafrominfo($adapter, string $tablename, $obj, $tabinfo)
{
    $c = null;
    if ($obj === null)
        $c = igk_db_create_row($tablename);
 ...
 * @deprecated
 */
function igk_db_create_emptyresult($ctrl)
{
    $g = igk_get_data_adapter($ctrl);
    $s = $g ? $g->createEmptyResult() : null;
    return $s;
}
/**
...
function igk_db_delete($controllerOrAdpaterName, $table, $where = null, $dbname = null, $leaveOpen = false)
{
    ...
    if ($adapt) {
        ...
        igk_db_close_ad($adapt, $leaveOpen);
        return $r;
    } else {
 ...
 * @param mixed $tablanename
 * @param mixed $callbackfilter
 * @return mixed
 */
 ...
 * @return mixed
 */
function igk_db_get_ctrl_tables(BaseController $ctrl)
{
...
function igk_db_sync_todb($ctrl, $adapter = null, $callinit = null)
{
    ...
    $tables = igk_db_get_ctrl_tables($ctrl);
    $ad->initForInitDb();
    $tc = $tables;
    $init = [];
    foreach ($tc as $v) {
 ...
 * @return void 
 */
function igk_db_close_ad($ad, $leaveOpen = false)
{
    !$leaveOpen && $ad->close();
}
/**
...
{
    $adapt = igk_get_data_adapter($controllerOrAdpaterName);
    if ($adapt) {
        $adapt->connect($dbname);
...
{
    if (!$result || !igk_reflection_class_implement($result, IQueryResult::class)) {
        return null;
    }
 ...
 * @return mixed
 */
function igk_debug_wln_i(string $tag, string $msg)
{
 ...
 * @param IGKHtmlDoc $doc  the document where to load
 * @param null|string|array $folder  string|array the folder of target scripts
 * @param mixed $tag
 * @param mixed $strict
 * @return mixed
 */
    ...
    if ($is_prod) {
        if (is_dir($folder)) {
            ...
            if (igk_io_cache_file_exists($f)) {
                ...
                $u = "!@res/Scripts/" . $uid;
                $doc->addTempScript($u . ($strict ? "?strict=1" . $options : ""))->activate("defer");
...
function igk_error_page404($msg)
{
    ...
    if (igk_io_file_exists($file)) {
        ...
        $trace_css = "";
        $buri = igk_io_baseuri();
        if (igk_io_file_exists($d = igk_io_resourcesdir() . "/Fonts/google/Roboto100,200,400,700,900.css")) {
...
function igk_eval_in_context($src, $ctrl, $raw)
{
    ...
    extract(igk_extract_ref(igk_get_context_args()));
...
if (!function_exists('igk_explode')) {
     ...
     * @return string[] 
     */
    function igk_explode(string $haystack, string $separator = '|')
    {
...
{
    if (is_null($obj))
        throw new ArgumentNotValidException('obj');
    if (is_string($list)) {
        $list = explode('|', $list);
    }
    $p = [];
 ...
 * @return void 
 */
function &igk_get_refvalue($obj, $k)
{
    $v = null;
    if (is_object($obj)) {
    ...
    } else if (is_array($obj) && key_exists($k, $obj)) {
        $v = &$obj[$k];
    } else {
        igk_die('missing reference object');
    }
    return $v;
}
...
if (!function_exists('igk_extract_obj')) {
    /**
     * helper : extract an convert to std classobject
     * @param mixed $obj
     * @param mixed $list
     * @return mixed
     */
...
function igk_get_defaultconfigdata()
{
    ...
    $data = [
        ...
        "cache_loaded_file" => 0,
        "company_name" => "igkdev",
        "copyright" => IGK_COPYRIGHT,
        "db_name" => "igkdev",
        "db_prefix" => IGK_DEFAULT_DB_PREFIX,
        "db_pwd" => "",
        "db_server" => "localhost",
        "db_user" => "root",
        "db_charset" => "utf8",
        ...
        "meta_keywords" => "IGKDEV, .NET, C#, BONDJE DOUE, Developper, PHP, HTML5, WEBDEV, PLATEFORM, BALAFON",
        "meta_title" => "igkdev.be",
        "powered_message" => "IGKDEV",
        "powered_uri" => "https://igkdev.com",
        "show_debug" => 0,
        "show_powered" => 1,
        "website_domain" => $servername,
        "website_prefix" => "igk",
        "website_title" => $servername,
        "default_lang" => "fr",
...
function igk_get_document(string $key, $clear = false, $init = false, ?callable $init_settings = null)
{
    ...
    $v_key = (string)IGK_KEY_DOCUMENTS;
    if (!isset($v_appInfo->{$v_key})) {
        $v_appInfo->{$v_key} = [];
    }
...
/**
 * shortcut: retrieve the active installed module informations
 * @return ?array <string name,stdClass info>
 */
 ...
 * @param ?closure|string|null $callbackfilter callable that will filter the request available key
...
/**
 * get system default user by login
 * @return mixed
 */
...
function igk_getbaseindex_src(string $libfile): string
{
    ...
    $inf->app_dir = (dirname($libfile) == IGK_APP_DIR) ? '' : $v_fc_resolv_dir($libfile, IGK_APP_DIR);
    ...
    $inf->comment = 'this file was generated by balafon service. please do not modify until you know what you are doing.';
    $inf->author_mail = IGK_AUTHOR_CONTACT;
    $inf->author = IGK_AUTHOR;
    $s = implode(
        "\n",
        array_merge(
            [
                ...
                "// @date : {$inf->date}",
                "// @author : {$inf->author}",
                "// @mail: {$inf->author_mail}",
                "// @generator: balafon base index.php service generator",
                "// @note: {$inf->comment}",
                "if (!version_compare(PHP_VERSION, '7.3', '>=')){",
                "    die('mandory version required. 7.3<=');",
                "}",
                ...
                "// + |",
                "define('IGK_APP_DIR', \$appdir" .
                    (empty($inf->app_dir) ? '' : ".'{$inf->app_dir}'")
                    . ");"
            ],
            $inf->sess_dir ?
                [
                    ...
                    "// + |",
                    "define('IGK_SESS_DIR', \$appdir.'{$inf->sess_dir}');",
                ] : [],
            [
                "// + |------------------------------------------------------------",
                "// + | define application projects directory",
                "// + |",
                "define('IGK_PROJECT_DIR', \$appdir.'{$inf->projects_dir}');",
                "unset(\$appdir);",
                "// + |------------------------------------------------------------",
                ...
                "// + |",
                "return IGKApplication::Boot(IGKApplication::WEB_TYPE)->run(__FILE__); ",
                "})();",
            ]
        )
    );
...
function igk_header_mime()
{
    ...
    $mime = igk_get_env("sys://mimetype") ?? array(
        ...
        "vue" => "text/javascript",
        "xsd" => $is_chrome ? "text/html" : "application/xml",
...
function igk_html_add_balafonjsscriptfile($doc, $file, $temp = 0)
{
    ...
    if (isset($g[$file])) {
        $b = $g[$file];
    } else if (igk_io_file_exists($file)) {
        $b = igk_create_node("balafonJS");
        if ($temp) {
...
function igk_html_article_bck(BaseController $ctrl, $name, $target, $data = null, $tagname = null, $forcecreation = true, $evalExpression = true, $articleoptions = true)
{
    ...
    if (!igk_io_file_exists($f)) {
        if (!empty($d) && ($d != ".") && is_dir($d) || (strpos($d, igk_io_basedir()) === 0)) {
            ...
            if (!igk_io_file_exists($name) && ($ctrl != null))
                $f = $ctrl->getArticle($name);
            else
                return;
        }
    }
    if ($forcecreation && !igk_io_file_exists($f)) {
...
function igk_html_article(BaseController $ctrl, $name, $target, ?array $data = null, $tagname = null, $forcecreation = true, $evalExpression = true, $articleoptions = true)
{
    ...
    if (!igk_io_cache_file_exists($f)) {
        if (!empty($d) && ($d != ".") && is_dir($d) || (strpos($d, igk_io_basedir()) === 0)) {
            ...
            if (!igk_io_cache_file_exists($name) && ($ctrl != null))
                $f = $ctrl->getArticle($name);
            else
                return;
        }
    }
    if ($forcecreation && !igk_io_cache_file_exists($f)) {
        igk_io_save_file_as_utf8_wbom($f, IGK_STR_EMPTY, true);
    }
    if (($target == null) && ($tagname == null)) {
        return;
    ...
    if (!is_dir($f) && igk_io_cache_file_exists($f) && ($content = igk_io_read_allfile($f))) {
 ...
 * @param bool $evalExpression
 * @param ?HtmlLoadingContextOptions $ldcontext
 * @param string $f
 * @param BaseController $ctrl
 * @param mixed $data
 * @param mixed $articleoptions
...
{
    $d = igk_create_node("div");
    if (!igk_io_file_exists($templateArticleName)) {
        if ($ctrl)
...
function igk_html_ctrl_view_config($ctrl, $target, $titlekey, $descfile = null)
{
    ...
    if ($descfile && igk_io_cache_file_exists($f = $ctrl->getArticle($descfile))) {
 ...
 * @param mixed $ctx
 * @param mixed $a
 * @param bool $transformEval
 * @return mixed
 */
function igk_html_databinding_treatresponse($rep, $ctrl, $raw, $ctx = null, $a = 0, bool $transformEval = false)
{
    if (!is_string($rep)) {
...
function igk_html_get_system_uri($link, $option = null, $webapp = null)
{
    ...
    if (!empty($link)) {
        ...
        if (igk_io_file_exists(Path::Combine($bdir, $link))) {
            return $link;
        }
        $s = "";
        $k = explode("?", $link);
        if (igk_io_file_exists($k[0])) {
...
{
    extract(igk_extract_ref(igk_extract_data(igk_getv(array_slice(func_get_args(), 1), 0, []))));
...
function igk_html_resolv_img_uri($path)
{
    ...
    if (!empty($f) && igk_io_file_exists($f)) {
        $r = igk_realpath($f);
        $s = igk_io_basepath($f);
        if ($s == $r) {
...
function igk_html_save_doc_formail($doc, $file)
{
    ...
    $opt->Context = HtmlRenderingContext::Mail;
 ...
 * @param mixed $raw current cibling row
 * @param mixed $ctrl controller
 * @param mixed $ctx
 * @return mixed
 */
 ...
 * @return mixed
 */
function igk_include_file(string $file, $args = null)
{
    if ($args) {
        foreach ($args as $k => $v) {
            $$k = &$args[$k];
...
{
    if (!igk_io_file_exists(func_get_arg(0), true))
        return null;
...
{
    if (is_string($file) && !igk_io_file_exists($file))
        return;
    $n = "";
 ...
 * @param IControllerInitListener $listener
...
{
    $r = @fopen($file, igk_io_file_exists($file) ? "a+" : "w+");
    if ($r) {
        fwrite($r, $data);
        fclose($r);
    } else {
 ...
 * @param mixed $params
 * @param mixed $callback
 * @param mixed $cacheout
 * @return mixed
 */
function igk_io_dispatch_file($dir, $params, $callback = null, $cacheout = 3600)
{
    if ((igk_count($params) > 0) && (igk_io_file_exists($f = $dir . "/" . implode("/", $params))) && (!$callback || $callback($f))) {
...
function igk_io_fullpath2fulluri($file)
{
    ...
    if ($ruri = IGKResourceUriResolver::getInstance()->resolve($file)) {
...
function igk_io_get_article_file($name, $dir, $lang = null)
{
    ...
    if (preg_match(IGK_ARTICLE_TEMPLATE_REGEX, $name))
        ...
        if (!igk_io_file_exists($file) && igk_io_file_exists($cf = $dir . "/" . $name . "." . IGK_DEFAULT_VIEW_EXT)) {
            $file = $cf;
        }
    }
    return $file;
...
/**
 * helper: retrieve uploaded data . \environement : set RequestFakeJsonInput
 * @param bool $usefaker
 * @return mixed
 */
...
{
    return igk_io_file_exists($f) && !is_dir($f);
}
/**
 * check if full path
 * @param mixed $d
...
function igk_io_moveuploadedfiletodatafolder($name, $dir, $pattern = "pics_%d%")
{
    ...
    if ($img) {
        if (IO::CreateDir($dir)) {
            if (!igk_io_file_exists($dir . "/.htaccess")) {
...
function igk_io_save_posted_file($id, $folder, $callback)
{
    ...
    if (($f["error"] == 0) && igk_qr_confirm()) {
        if (IO::CreateDir($folder)) {
            ...
            if (!igk_io_file_exists($destination)) {
...
/**
 * return the system full path according to BASEDIR.convert file to uri offline presentation
 * @param mixed $f
 * @param mixed $exist
 * @return mixed
 */
...
/**
 * get previous copntent name
 * @param mixed $filename
 * @param mixed $to_line
 * @return mixed
 */
function igk_io_prev_content($filename, $to_line)
{
    return implode("\n",  array_slice(explode("\n", file_get_contents($filename)), 0, $to_line));
}
/**
 * auto generate doc.
 * @param mixed $f
...
function igk_js_dist_scripts($files)
{
    ...
    foreach ($files as $file) {
        if (!igk_io_file_exists($file))
            continue;
        ob_start();
        $mergescallback($file, igk_io_basepath($file));
        $output = ob_get_contents();
        ob_clean();
        $out .= $output . IGK_LF;
    }
 ...
 * @param mixed $op
 * @param mixed $doc
 * @deprecated use a lmodule that will
 * @return mixed
 */
function igk_js_enable_tinymce($target, $elements = null, $op = null, $doc = null)
{
    $c_tinymce_class = 'tinymce';
    if (!($mce = igk_require_module($c_tinymce_class,  ActionHelper::Nothing(), true, 0))) {
        return;
    }
...
function igk_js_load_found_script($doc, $dirname)
{
    ...
    foreach (IO::GetFiles($dirname, '/\.js$/i') as $g) {
        $doc->addScript($g);
    }
    return true;
}
...
function igk_js_load_script(IGKHtmlDoc $doc, $dirname, $tag = 'priv', $regtype = 0)
{
    ...
    if (igk_io_file_exists($cache_path)) {
        $data = include($cache_path);
        foreach ($data as $f => $vtag) {
            $f = igk_io_expand_path($f);
            $res_files[$f] = $vtag;
            if (igk_io_path_ext($f) == "js") {
                $doc->addScript($f, $vtag);
            } else {
                $callback($f);
            }
        }
    ...
    while (($classname = array_shift($tab)) !== null) {
        if (!class_exists($classname, false)) {
            ...
            foreach ($resolv_class as $version) {
                if (igk_io_file_exists($cf = igk_uri($cdir . "/" . $f . "{$version}.php")) || (!empty($version) && igk_io_file_exists($cf = igk_uri($cdir . "/{$version}/" . $f . "php")))) {
                    require_once($cf);
...
/**
 * return font codenavigate to session redirection parameter
 * @return mixed
 */
...
function igk_pattern_matcher_matchcallback($m)
{
    ...
    switch (strtolower($n)) {
        case 'verbs':
            ...
            $tm = "(?P<" . $n . ">(get|post|options|head|put|copy|patch|delete))";
            break;
        case "options":
            // + | ----------------------------------------------
            // + | options matching 
...
/**
 * shorcut: get Handler info -> IViewHandlerInfo
 * @return mixed
 */
function igk_view_handler_info()
{
    return ViewHelper::GetHandlerInfo();
}
/**
    ...
    if ($c && !method_exists($ctrl, $c)) {
        ...
        if (is_dir($dir)) {
            ...
            while (!$found && (count($param) > 0)) {
                ...
                if (!is_dir($file)) {
                    if (preg_match($ext_regex, $file) && igk_io_cache_file_exists($file)) {
                        $found = true;
                        $c .= "/" . $path;
                        $param = [];
                    } else {
                        foreach ($exts as $ext) {
                            if (igk_io_cache_file_exists($cf = $file . $ext)) {
                                $c .= "/" . $path;
                                $param = [];
                                $found = true;
                                break;
                    ...
                    $file = rtrim($file, "/");
                    if (igk_io_cache_file_exists($cf = $file . "/" . IGK_DEFAULT_VIEW_FILE)) {
                        $c .= rtrim("/" . $path, "/");
                        $param = [];
                        $found = true;
                        $file = $cf;
 ...
 * @param ?closure $callback use to register package
...
/**
 * helper: read values form string 
 * @param string $data
 * @param string $value_delimiter
 * @param string $definition_delimiter
 * @return array<string,string> 
 */
function igk_regex_read_value(string $data, string $value_delimiter = ':', $definition_delimiter = ';')
{
    $container = new RegexMatcherContainer;
    $container->match($value_delimiter, "value");
    $container->match($definition_delimiter, "def");
    $tab = [];
    $lpos = 0;
    $skey = [];
    $container->treat($data, function ($g, &$int) use (&$lpos, &$tab, $data, &$skey) {
        $s = substr($data, $lpos, $g->from - $lpos);
        if ($g->tokenID == 'value') {
            $skey[] = trim($s);
        } else {
            if ($skey) {
...
{
    if (igk_io_file_exists($f = igk_uri($ctrl->getResourcesDir() . "/Img/{$resname}"))) {
        return $f;
    }
    return null;
}
...
if (!function_exists('igk_str_transform_linefeed')) {
     ...
     * @param ?array $feed feed expression
     * @return string
     */
 ...
 * @return mixed
 */
function igk_str_get_version(string $version)
{
    $tb = explode(".", $version);
    if (igk_count($tb) < 4) {
        $c = igk_count($tb) - 4;
        ...
        igk_wln_e(__FILE__ . ":" . __LINE__, 'shifting');
        $tb = array_slice($tb, 0, 4);
    }
 ...
 * reprease string vak
 * @param string $p
 * @param int $c times to repeat
 * @return mixed
 */
function igk_str_repeat(string $p, int $c)
{
    $o = "";
    while ($c > 0) {
        $o .= $p;
 ...
 * @return mixed
 */
function igk_str_startwith(string $str, string $pattern)
{
 ...
 * @return mixed
 */
function igk_svg_bind_svgs($doc, string $dir = IGK_LIB_DIR . "/Data/R/svg/icons")
{
    igk_environment()->isDev() && igk_ilog('bind svgs : ' . $dir);
...
function igk_svg_bindfile($name, $file)
{
    ...
    if ($source && igk_io_file_exists($file)) {
...
{
    if (!empty($name)) {
        if (igk_io_file_exists($f = $dir . "/" . $name . ".svg")) {
...
function igk_sys_balafon_js(?BaseController $controller = null, $debug = false, bool $minify = true, $usecache = true)
{
    ...
    $header .= "'use strict';";
    $s = preg_replace("/('|\")use strict\\1(\s*;)*/", '', $src);
    if (ob_get_level() > 0)
        ob_clean();
    ob_start();
    // + | $s = inject module reference settings globaly 
    $s = sprintf('(function(window){%s})(window);', implode("\n", [
        "const __module_refs = [];",
        $s
    ]));
    $src = "";
...
function igk_sys_config_view($file)
{
    ...
    if (!igk_get_env("sys://data/configsettings") && igk_io_cache_file_exists($f = igk_io_basedatadir("/configure"))) {
    ...
    if (!igk_environment()->IsWebApp() && ($p = igk_sys_getconfig("configuration_port")) && ($p != $_SERVER["SERVER_PORT"])) {
        if ($p == 443) {
            $s = igk_secure_uri(igk_io_baseDomainUri(), true, false) . $v_conf_path;
        } else {
    ...
    $igk = igk_app();
    if (!$igk) {
        igk_log_write_i("CONFIG", "No Instance found for configuration page");
        igk_exit();
    }
    try {
        ...
        if ($doc) {
            ...
            igk_header_no_cache();
            $doc->setBaseUri(igk_io_baseuri() . "{$v_conf_path}/");
            $doc->Favicon = new IGKHtmlRelativeUriValueAttribute(IGK_LIB_DIR . "/Default/R/Img/cfavicon.ico");

            \IGK\System\Core\Configuration\CPanel::SetupDocument($doc);
            igk_hook(IGKEvents::FILTER_PRE_CPANEL_BEFORE_RENDER, ['doc' => $doc, 'ctrl' => $cnf]);
...
/**
 * get current registrated user - die current user not set
 * @return mixed
 */
...
function igk_sys_getall_funclist($ctrl)
{
    ...
    $cl = get_class($ctrl);
    if (igk_io_file_exists($f) && ($s = HtmlReader::LoadFile($f))) {
...
function igk_sys_handle_entry_file($dir)
{
    ...
    $e = 0;
    foreach ($tab as $k) {
        if (igk_io_file_exists($ext = $dir . "/" . trim($k))) {
            $e = 1;
            break;
        }
    }
...
function igk_sys_handle_request($uri)
{
    ...
    if (igk_io_file_exists($f = IGK_APP_DIR . $furi) || (igk_phar_running() && igk_io_file_exists($f = Phar::running() . $furi))) {
        if (is_dir($f)) {
    ...
    switch ($m) {
        case "DELETE":
            if (igk_getv($upd, "destroysession") == 1) {
                ...
                $id = session_id();
                igk_session_destroy();
                igk_json(json_encode(["sessiondestroy" => 1, 'sessid' => $id]));
            }
            igk_exit();
            break;
    }
...
{
    return igk_io_cache_file_exists(igk_io_cachedir() . "/" . IGK_CACHE_HTML);
}
/**
 * auto generate doc.
 * @param mixed $page
...
function igk_sys_reg_componentname($tab)
{
    ...
    if (igk_count($ttab) == 0) {
        if (igk_io_file_exists(IGK_COMPONENT_NAMESFILE)) {
            $f = function (&$htab) {
...
function igk_sys_reg_referencedir($src, $dir)
{
    ...
    if (igk_io_file_exists($src, true))
...
function igk_sys_register_user($u, $gooduri = null, $baduri = null, $listener = null)
{
    ...
    $tb = $model::table();
    $r = IGK\Models\Users::createEmptyRow();
    if (empty($r->clLogin)) {
...
function igk_sys_show_error_doc($code, $defctrl = null, $callback = null)
{
    ...
    if ($defctrl) {
        if (igk_io_file_exists($f = $defctrl::GetErrorView($code))) {
    ...
    if (igk_io_file_exists($codefile)) {
        extract($c_args);
        include($codefile);
    } else switch ($code) {
        case 403:
    ...
    if (is_callable($callback))
        $callback($doc);
    $opt = HtmlRenderer::CreateRenderOptions();
    $opt->Context = HtmlRenderingContext::Html;
    $doc->renderAJX($opt);
}
/**
 * system is showdown
...
function igk_sys_start_engine($file, $render = true)
{
    try {
        ...
        IGKApplication::Boot(IGKApplication::WEB_TYPE)
            ->run($file, $render);
    } catch (\Exception $ex) {
        if (igk_environment()->isDev()) {
...
function igk_sys_zip_core(string $tfile, $incVersion = false, $verbose = false)
{
    if (!class_exists(ZipArchive::class, false))
        return false;
    $zip = new ZipArchive();
    if ($zip->open($tfile, ZipArchive::CREATE | ZipArchive::OVERWRITE)) {
 ...
 * @return mixed
 */
function igk_use_component_package(string $packagename): bool
{
    $key = Constants::COMPONENT_PACKAGE_KEY;
    $tab = igk_get_env($key);
 ...
 * @return mixed 
 */
function igk_component_packages()
{
    return igk_environment()->get(Constants::COMPONENT_PACKAGE_KEY);
}
/**
 * define usage of required package list
 * @param mixed $packageList
 * @return mixed
 */
...
/**
 * retrieve web knows colors
 * @return mixed
 */
function igk_web_colors()
{
    static $tab = null;
    if (is_null($tab)) {
        $tab = call_user_func(function () {
            $cl = [];
            include IGK_LIB_DIR . '/Styles/webknowcolor.pcss';
            return $cl;
        });
    }
    return $tab;
...
function igk_xml_create_to_node_settings()
{
    return (object)array(
        "storeAttribAsNode" => 0,
        "handleArray" => function ($k, $tv, $n, $setting = null) {
            foreach ($tv as $sk => $v) {
                $nv = $n->addXmlNode($k);
            ...
            case "!":
                if ($m == 1) {
                    if (substr($inf->content, $inf->offset + 1, 2) == "--") {
                        ...
                        $inf->offset += 8;
                        igk_xml_read_doctype($inf->content, $inf->offset);
                    } else {
