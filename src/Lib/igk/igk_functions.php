...
use IGK\System\Html\Dom\IGKHtmlTextNode;
 ...
 */
function igk_html_uri($u)
{
    return \IGK\Helper\StringUtility::Uri($u);
}
...
function igk_ajx_update($uri, $target = 'body', $type = 'get')
{
    ...
    igk_createnode("script")->setAttribute("autoremove", "true")->setContent("ns_igk.ajx.{$type}('$uri', null, {$target});")->renderAJX();
}
...
/**
 * Create a reference assoc key in define object
...
{
    $bvtheme = new HtmlDocTheme($doc, "temp://files");
    $out = "";
...
{
    return HtmlNode::CreateWebNode($tag, $attributes, $index);
}
///<summary></summary>
///<param name="args" default="null"></param>
/**
...
///<summary>Represente igk_createnotagnode function</summary>
/**
...
{
    $c = new IGKXmlDocument($tagName, $docType);
    return $c;
}
 ...
 */
function igk_create_xmlprocessor($type)
{
    return new  \IGK\System\Html\XML\XmlProcessor($type);
}
...
///<summary>Represente igk_css_add_doc_style function</summary>
/**
 * Represente igk_css_add_doc_style function
 * @deprecated init document in initilize 
 */
...
{ 
    $selector = array(); 
    foreach (igk_app()->getDoc()->SysTheme->def->getAttributes() as $k => $v) {
        $tab = explode(',', $k);
        if (empty($v))
            continue;
        foreach ($tab as $s => $t) {
    ...
    foreach (igk_app()->getDoc()->Theme->def->getAttributes() as $k => $v) {
        $tab = explode(',', $k);
        if (empty($v))
            continue;
        foreach ($tab as $s => $t) {
...
function igk_css_get_style($classname)
{
    ...
    if ($igk === null)
        return;
    $b = $igk->getDoc()->Theme[$classname];
    if ($b == null) {
        igk_ilog("/!\\ style not found : " . $classname);
        $b = $igk->getDoc()->SysTheme[$classname];
        if ($b) {
            $o = igk_css_treat($igk->getDoc()->SysTheme, $b);
            return $o;
        }
    } else {
        $o = igk_css_treat($igk->getDoc()->Theme, $b);
        return $o;
    }
    return IGK_STR_EMPTY;
}
...
function igk_css_load_theme($th = null)
{
    ...
    if (!$r)
        return;
    $th = $th ? $th : igk_app()->getDoc()->SysTheme;
    $t = array();
    $t["cl"] = &$th->def->getCl();
...
{
    $vtemp = new HtmlDocTheme(igk_app()->getDoc(), "theme://inline/tempfiles");
...
{
    return "sys://ctrl/" . sh1(get_class($ctrl));
}
///<summary></summary>
///<param name="ctrl"></param>
/**
...
function igk_db_backup_ctrl($ctrl, $defentries = 1)
{
    if ($ctrl) {
        ...
        if ($apt->connect()) {
            ...
            foreach ($tb as $v) {
                $rep = $schema->addXmlNode(DbSchemas::DATA_DEFINITION)->setAttributes(array("TableName" => $v));
                if ($defentries)
    ...
    foreach (igk_sys_getall_ctrl() as $k) {
        ...
        if (isset($v_info["data-schema"]) || is_array($v_info) && is_array(igk_getv(array_values($v_info), 0))) {
            foreach ($v_info as $table => $rinfo) {
                ...
                $info = (object)array(
                    "tableName" => $table,
                    "tableInfo" => IGKDbColumnInfo::AssocInfo(
                        $rinfo,
                        $table
                    ),
                    "Ctrl" => $k,
...
/**
 * Represente igk_db_init_dataschema function
 * @param mixed $ctrl 
...
function igk_db_load_data_schema_array($n, &$tables, &$tbrelations = null, &$migrations = null, $ctrl = null, $resolvname = true, $reload = false)
{
    ...
    $tentries = [];
    if ($entries) {
        foreach ($n->getElementsByTagName(DbSchemas::ROWS_TAG) as $v) {
            if ($tb = $v["For"]) {
    ...
    foreach ($n->getElementsByTagName(DbSchemas::DATA_DEFINITION) as $v) {
        ...
        foreach ($v->getElementsByTagName(IGK_COLUMN_TAGNAME) as $vv) {
            $cl = IGKDbColumnInfo::CreateWithRelation(igk_to_array($vv->Attributes), $tb, $ctrl, $tbrelations);
            $c[$cl->clName] = $cl;
        }
        $tables[$tb] = array(
            "ColumnInfo" => $c,
            "Description" => igk_getv(
                $v,
                "Description"
            ),
            "Entries" => igk_getv(
...
function igk_db_load_data_schemas($file, $ctrl = null, $resolvname = true)
{
    ...
    return igk_db_load_data_schemas_node(HtmlReader::LoadFile($file), $ctrl, $resolvname);
}
...
///<summary>Represente igk_do_response function</summary>
///<param name="r"></param>
/**
 * Represente igk_do_response function
 * @param mixed $r 
 */
...
/**
 * Represente igk_get_balafonjs_src function
 */
...
function igk_get_data_adapter($controllerOrAdpaterName, $throwException = false)
{
    ...
    if (is_string($controllerOrAdpaterName)) {
        ...
        if ($controllerOrAdpaterName instanceof IGKDataAdapter)
...
function igk_get_defaultconfigdata()
{
    ...
    $data = ["admin_login" => "admin", "admin_pwd" => "21232f297a57a5a743894a0e4a801fc3", "allow_article_config" => 0, "allow_auto_cache_page" => 0, "allow_debugging" => 0, "allow_log" => 0, "app_default_controller_tag_name" => "div", "cache_loaded_file" => 0, "company_name" => "igkdev", "copyright" => IGK_COPYRIGHT, "db_name" => "igkdev", "db_prefix" => IGK_DEFAULT_DB_PREFIX, "db_pwd" => "rootbonaje", "db_server" => "local_mysql", "db_user" => "root", "default_controller" => "igk_default", "mail_admin" => "administrator@igkdev.com", "mail_contact" => "info@igkdev.com", "mail_port" => 25, "mail_server" => "relay.skynet.be", "menu_defaultPage" => "default", "meta_copyright" => "igkdev@igkdev.com", "meta_description" => "default page description", "meta_enctype" => "text/html; charset=utf-8", "meta_keywords" => "IGKDEV, .NET, C#, BONDJE DOUE, Developper, PHP, HTML5, WEBDEV, PLATEFORM, BALAFON", "meta_title" => "igkdev.be", "powered_message" => "", "show_debug" => 0, "show_powered" => 1, "website_domain" => $servername, "website_prefix" => "igk", "website_title" => $servername, "default_lang" => "fr",] + include(IGK_LIB_DIR . "/.setting.global.pinc");
 ...
 * @param mixed $callbackfilter callable that will filter the request available key
 */
...
{
    $server_n = igk_io_dir(igk_server_name());
    $rdir = igk_io_rootdir();
    $bdir = igk_io_basedir();
...
function igk_html_article($ctrl, $name, $target, $data = null, $tagname = null, $forcecreation = true, $evalExpression = true, $articleoptions = true)
{
    ...
    $ldcontext->_request = __FUNCTION__;
...
{
    if (($p = igk_get_env(IGK_XML_CREATOR_PARENT_KEY)) != null) {
        array_pop($p);
...
function igk_html_render_node($n, &$options, $tab = null, $textonly = false, $chain = 1)
{
    ...
    if (!($c_options = igk_environment()->render_option)) {
        $c_options = [];
    }
    array_push($c_options, $options);
    igk_environment()->render_option = $c_options;
    $count_while = 0;
    while ($q && !$options->Stop) {
        $count_while++;

        if ($count_while > 1000) {
            die("infine loop detected");
        }

        if (property_exists($options, "Engine")) {
            $options->lastRendering = $q;
            $s .= $options->Engine->render($q, $options);
            $q = array_pop($tab);
            continue;
        }
        $closep = false;
        if ($p && ($p === $q)) {
            ...
            if (is_object($q) && !is_subclass_of($q, HtmlItemBase::class) && !igk_getv($q, "subrendering")) {
        ...
        if (!$closep) {
            ...
            $c = HtmlUtils::GetContentValue($q, $options);
            $inner .= $c;
            if ($c_tchild > 0) {
                if ($updatedepth) {
                    $options->Depth++;
...
///<summary>Represente igk_html_skip_comment function</summary>
///<param name="c"></param>
///<param name="pos" ref="true"></param>
/**
 * Represente igk_html_skip_comment function
 * @param mixed $c 
 * @param mixed $pos 
 */
 ...
 * @param controller $$ctrl  the controller that request . it can be null
 * @param mixed $$raw  string or context array definition
 * @param node $$target  node for requesting . it can be null
 */
...
function igk_ilog_file()
{
    ...
    if (!$f) {
        $f = igk_io_dir(igk_io_applicationdir() . "/" . IGKLog::LOGFILE);
    }
    return $f;
}
///<summary></summary>
...
function igk_include_view_file($ctrl, $file)
{
    ...
    if (0 && !$cache->cacheExpired($file)) {
        ...
        // igk_html_article($ctrl, $file, $node, $args, $tagname=null, false, true, false);
        // ob_start();
        ...
        // $extra = empty($src)? ltrim($extra): $extra;
        // igk_io_w2file($_f, $src.$extra);
        // array_unshift($args, $_f);
    ...
    try {
        ...
        if (igk_environment()->is("DEV")) {
            ...
            igk_wln_e(
                "Include view Error : " . igk_environment()->last($key),
                $ex->getMessage(),
                $ex->getTrace()
            );
        }
...
{
    HtmlItemBase::$BasicMethod = array(
 ...
 * @param mixed $path 
 */
function igk_init_module($path,  ?callable $init = null)
{
...
{
    return IGKAppSystem::InitEnv($dirname);
}
///<summary></summary>
...
///<param name="path"></param>
/**
 * Represente igk_io_access_path function
 * @param mixed $path 
 */
...
function igk_io_corestyle_uri()
{
    ...
    // igk_io_baseuri(IGK_RES_FOLDER."/".IGK_STYLE_FOLDER."/balafon.css?v=".IGK_VERSION);
}
///<summary></summary>
///<param name="ctrl"></param>
/**
...
{
    return IO::GetRelativePath($dir, getcwd());
}
///<summary></summary>
/**
 * 
...
{
    return igk_html_uri(igk_str_rm_last(IO::GetRootUri(), "/") . "/" . igk_str_rm_last(igk_getv(explode('?', ltrim(igk_io_request_uri(), '/')), 0), '/'));
}
///<summary></summary>
///<param name="$c"></param>
...
function igk_io_getrelativepath($source, $target)
{
    ...
    if ($i !== false) {
        $o = "./" . ltrim(substr($v, $i + strlen($sdir)), '/');
    } else {
        $o = $v;
        $i = strpos($v, $sdir);
        $p = "../";
        ...
        if ($sdir) {
            $o = igk_io_dir($p . ltrim(substr($v, $i + strlen($sdir)), '/'));
        }
    }
    return $o;
}
...
{
    return igk_io_dir(implode(DIRECTORY_SEPARATOR, $args));
}
///<summary></summary>
///<param name="file"></param>
///<param name="options" default="null"></param>
...
function igk_io_projectdir()
{
    ...
    if ($pdir == null) {
        if (defined($key)) {
            ...
            $pdir = igk_getv($_SERVER, $key, igk_io_applicationdir() . "/" . IGK_PROJECTS_FOLDER);
...
///<param name="link"></param>
/**
 * Represente igk_io_relativepath function
 * @param mixed $spath 
 * @param mixed $link 
 */
function igk_io_relativepath($path, $link)
{
...
function igk_io_save_file_as_utf8_wbom($filename, $content, $overwrite = true, $chmod = IGK_DEFAULT_FILE_MASK, $type = "w+")
{
    ...
    fclose($hf);
    if ($chmod) {
        if (in_array(PHP_OS, ["Linux"]) && ($user = posix_getpwuid(fileowner($filename))) && (get_current_user() == $user["name"]) && !@chmod($filename, $chmod)) {
...
{
    $App = igk_app();
    return (IGKServer::IsLocal() && ($App->Configs->allow_debugging ? $App->Configs->allow_debugging : false));
}
///<summary></summary>
///<param name="ctrl"></param>
/**
...
{
    // igk_wln_e("file : ". IGKXmlNode::class);
    if ($n instanceof IGKXmlNode)
        return false;
...
{
    return ($g = igk_get_env(IGKEnvironment::VIEW_INC_VIEW)) && isset($g[igk_io_dir($file)]);
}
...
{
    if (!($mce = igk_require_module("tinymce", null, true, 0))) {
        return;
    }
 ...
 */
function igk_js_init_doc(HtmlDocumentNode $doc)
{
    $key = "loadingscript";
    ...
    $r = 1;
    if (!(!IGKServer::IsLocal() && file_exists($cache))) {
...
{
    // igk_debug_wln("load environment : " . $dirname);
    $t_files = array();
...
{
    return igk_const_defined("IGK_WRITE_LOG") || IGKServer::IsLocal();
}
///<summary></summary>
///<param name="msg"></param>
/**
 ...
 * @param mixed $tab 
 */
function igk_map_array_to_str($tab)
{
    $m = "";
    array_map(
        function ($v, $k) use (&$m) {
...
{
    $ns = implode("", array_map("ucfirst", explode("-", $ns)));
...
{
    static $autoloading = null, $reg_func, $sort;
    $k = "sys://autoload/func";
    if (is_callable($func)) {
        $c = igk_get_env($k);
        if (!$c) {
    ...
    if ($autoloading === null) {
        spl_autoload_register(function ($n) use ($k, &$sort, &$reg_func) {
            // igk_is_debug() && igk_ilog("try load " . $n . " : " . igk_sys_request_time());
            if ($reg_func) {
                $b = igk_get_env($k);
                if ($b) {
                    if ($sort) {
...
function igk_register_temp_uri($controllerClass)
{
    ...
    $is_module = is_object($controllerClass) && ($cl == IGKAppModule::class) && strstr($controllerClass->getDeclaredDir(), igk_get_module_dir(),);
...
function igk_require_module($modulename, callable $init = null, $loadall = 1, $die = 1)
{
    ...
    $IGK_ENV = igk_environment();
    $g = igk_get_env($v_k, array());
    $mkey = igk_html_uri(strtolower($modulename));
    if (isset($g[$mkey])) {
        return $g[$mkey];
    }
    ...
    if (file_exists($dir)) {
        ...
        if ($loadall) {
            $f = igk_io_getfiles(
                $dir,
                function ($c, &$excludedir = null) use ($ext_regex, $exclude_files) {
...
///<summary>shortcut to IGKServer::IsLocal()</summary>
/**
 * shortcut to IGKServer::IsLocal()
 */
...
{
    return implode("", array_map("ucfirst", array_filter(explode("_", str_replace(" ", "_", str_replace("/", "\\", $n))))));
}
...
{
    list($v, $pipe)
        = igk_str_pipe_args($src, $c, $removequote);
...
///<summary>Represente igk_str_remove_quote function</summary>
///<param name="v"></param>
/**
 * Represente igk_str_remove_quote function
 * @param mixed $v 
 */
...
///<summary>Represente igk_str_uncollapsestring function</summary>
///<param name="v"></param>
/**
 * Represente igk_str_uncollapsestring function
 * @param mixed $v 
 */
...
function igk_svg_bindfile($name, $file)
{
    ...
    if ($source && file_exists($file)) {
        ...
        // if (!file_exists($cf = igk_io_resolvpath($file, igk_io_basedir()))) {
        //     if (!igk_sys_env_production()) {
        //         igk_debug_wln_e("not resolved  ", $file, $cf);
        //         throw new IGKException("something bad happend. svg file to bind not resolved " . $file . " " . $cf);
        //     }
        //     return;
        // }
        $f[$name] = $file;
...
function igk_svg_register_icons($doc, $name = null, $dir = IGK_LIB_DIR . "/Data/R/svg/icons")
{
    ...
    if ($cfiles = IO::GetFiles($dir, "/\.svg$/i")) {
        foreach ($cfiles as $v) {
            igk_svg_register($doc, igk_io_basenamewithoutext($v), $v);
        }
    }
    igk_wln_e("dfile ,lll ", $cfiles);
}
...
function igk_sys_balafon_js($doc, $debug = false)
{
    ...
    $accept = array_map("trim", explode(",", igk_server()->HTTP_ACCEPT_ENCODING));
...
{
    return HtmlItemBase::IsComponent($classname);
}
///------------------------------------------------------------
///sys function
...
{
    return HtmlItemBase::IsElement($classname);
}
...
function igk_sys_gen_sitemap($ctrl, $domain = "", $render = 1)
{
    ...
    $n->add($link);
    if (file_exists($f)) {
        $u = IGKResourceUriResolver::getInstance()->resolve($f);
...
{
    return RequestHandler::getInstance()->handle_ctrl_request_uri($u, $defaultBehaviour);
}
...
function igk_sys_regsyscontroller($className)
{
    ...
    if (class_exists($className)) {
        if ($g) {
            ...
            igk_environment()->push("sys://reg_controller", $className);
        }
    }
}
...
function igk_sys_render_index($file, $render = 1)
{
    ...
    igk_environment()->is("OPS") && igk_cache_handle();
    igk_clean_globals();
    $bdir = dirname($file);
    $srv = igk_server();
    if (!defined('IGK_APP_DIR')) {
    ...
    if (!$_start_index && file_exists($f = dirname($file) . $ch) && !is_dir($f)) {
        ...
        if (strstr(realpath($f), igk_io_cachedir() . "/dist/js/")) {
            if (igk_server()->accepts(["gzip", "deflate"])) {
    ...
    IGKApp::StartEngine($file, $render);
}
///<summary>Represente igk_sys_request_time function</summary>
/**
 * Represente igk_sys_request_time function
 */
...
///<summary>Represente igk_sys_zip_core function</summary>
///<param name="tfile"></param>
/**
 * Represente igk_sys_zip_core function
 * @param mixed $tfile 
 */
function igk_sys_zip_core($tfile)
{
    ...
    if ($zip->open($tfile, ZIPARCHIVE::CREATE)) {
        igk_zip_dir(IGK_LIB_DIR, $zip, "Lib/igk", "/(Lib\/igk\/temp|Lib\/igk\/application)|(\.(vscode|git(ignore)?|gkds|DS_Store|bak)$)/");
...
function igk_sys_zip_project($controller, $path, $author = IGK_AUTHOR)
{
    ...
    $ignore = "/(\/(temp|node_modules))|\.(vscode|git(ignore)?|gkds|DS_Store)$/";
    $zip = new ZipArchive();
    if (file_exists($path)) {
        @unlink($path);
    }
...
function igk_temp_bind_attribute($reader, $attr, $value, $context = null, $storecallback = null)
{
    ...
    if (isset($g[$attr])) {
        $inf = $g[$attr];
        list($k, $v)
            = $inf($reader, $attr, $value, $context, $storecallback);
...
function igk_user_build_info($r, $inf, $v, $ctrl)
{
    return (object)array(
        ...
        "clValue" => ($r->clType == 0) ? igk_db_table_select_where(
            $r->clDataType,
...
{
    $extra = "\n// + | view file: ";
    $extra .= (igk_environment()->is("DEV") ? $file : igk_io_collapse_path($file)) . "\n";
...
function igk_view_handle_obj_action($fname, $object, array $params = [], $exit = 1, $flag = 0)
{
    ...
    if (!empty($action) && (((($flag & 1) == 1) || method_exists($object, $action)) || igk_getv($object, "handleAllAction"))) {
        ...
        if (($g->getNumberOfRequiredParameters() == 1) && ($cl = $g->getParameters()[0]->getType()) && ($cl->getName() === IGK\System\Http\Request::class)) {
...
{
    $p = igk_create_xmlprocessor("xml-stylesheet");
    $xml = igk_create_xmlprocessor("xml");
    $xml["version"] = "1.0";
    $xml["encoding"] = "utf8";
