...
function igk_bind_host_css_style_file($file, $doc, $host)
{
    ...
    $sys = $doc->SysTheme;
    igkOb::Start();
    igk_css_bind_file($doc, $host, $file, $bvtheme);
 ...
 * @param null|int|array order index or node required parameter
 ...
 */
function igk_css_request_ctrl(?string $ref = null, ?string $uri = null)
{
    $ctrl = null;
 ...
 * @param mixed $dir 
 */
function igk_css_balafon_index($dir, $debug = null)
{
    ...
    // + | --------------------------------------------------
    // + | reset config mode
    if ($ref && igk_app()->settings->appInfo->config && 
    !StringUtility::UriStart($ref, $cnfPath)){
    ...
    if (
        ...
        (empty($ref) || StringUtility::UriStart($ref, $cnfPath))
    ) {
        ...
        if (!$ctrl && $ref) {
            ...
            if (igk_environment()->is("OPS") && igk_configs()->allow_page_cache) {
...
//  igk_wln_e("detect : ".$ctrl, igk_app()->settings->appInfo->config);
    // $v_css_conf = false;
    // + | ----------------------------------------------------
    ...
    if ($debug) {
        echo ("/* document is sys doc ? ".$doc->getIsSysDoc()."*/\n"); 
        echo ("/* referer : " . $ref . "*/ \n");
    ...
    if ($doc) {
        ...
        // igk_ilog(["files",json_encode($v_binTempFiles),  json_encode($v_tempFiles)]);
        // $seridata = serialize($vtheme);
 ...
 */
function igk_css_bind_file(IGKHtmlDoc $doc, ?BaseController $ctrl, string $f, ?HtmlDocTheme $theme = null)
{
    if (!file_exists($f)) {
...
function igk_css_bind_sys_global_files($theme = null, $bindfile = 1)
{
    ...
    foreach (array_keys($tab) as $d) {
        if (file_exists($d)) {
            igk_css_bind_file($doc, null, $d, $theme);
        }
    }
    if ($bindfile)
        igk_css_bind_theme_files($doc, $theme);
}
///<summary></summary>
///<param name="th"></param>
...
function igk_css_bind_theme_file(IGKHtmlDoc $doc, $th, $files)
{
    ...
    foreach ($lfile as $d) {
        ...
        if (!isset($v_lfiles[$file]) && !empty($d) && file_exists($file)) {
            igk_css_bind_file($doc, igk_getctrl($ctrl, false), $file, $th);
            $v_lfiles[$file] = 1;
        }
    }
}
 ...
 */
function igk_css_bind_theme_files(IGKHtmlDoc $doc, $theme, ?string $files = null)
{
    ...
    foreach ($lfile as $d) {
        ...
        if (file_exists($file)) {
            ...
            igk_css_bind_file($doc, $ctrl, $file, $theme);
        } else {
...
function igk_css_ob_get_tempfile($f, &$from = null)
{
    ...
    IGKOb::Start();
    igk_css_bind_file($doc, null, $f, $vtemp);
    $m = IGKOb::Content();
    $out = "";
    IGKOb::Clear();
    if (!empty($m)) {
 ...
 * 
 */
function igk_css_render_balafon_style(IGKHtmlDoc $doc, bool $no_systheme = false)
{
    ob_start();
    $o = "";
    if (!$no_systheme) {
...
function igk_css_render_style($tab, $doc = null)
{
    ...
    igk_css_bind_theme_files($doc, $th);
    $o = "";
    foreach ($tab as $k => $v) {
 ...
 */
function igk_ctrl_bind_css_file(\IGK\Controllers\BaseController $ctrl, \IGKHtmlDoc $document, string $file, $temp = 0)
{
    \IGK\System\Html\Css\CssUtils::InitBindingCssFile($ctrl, $document, $file, $temp);
}
 ...
 * @param mixed $uri 
 */
function igk_io_get_relative_currenturi(?string $uri = null)
{
...
function igk_push_article_chain($f, $context = null)
{
    ...
    $ctx = $context;
    $b = igk_get_env($key);
    // if (igk_environment()->isDev() && !isset($ctx->raw)){
    //     igk_trace();
    //     igk_exit();
    // }
...
function igk_sys_gen_sitemap($ctrl, $domain = "", $render = 1)
{
    ...
    if ($source = igk_configs()->getConfig("sitemap_xsl")) {
...
function igk_sys_zip_project($controller, $path, $author = IGK_AUTHOR, ?array $manifestOptions = [])
{
    ...
    if ($zip->open($path, ZIPARCHIVE::CREATE)) {
        ...
        $prjname = is_string($controller) ? basename($g) :
            igk_str_snake(basename(igk_io_dir(get_class($controller))));
